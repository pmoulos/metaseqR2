...
if ("biodist" %in% qcPlots) {
	...
	log <- capture.output({
		jsonList <- diagplotNoiseq(normGenesExpr,sampleList,covars=covarsStat,
			whichPlot="biodist",biodistOpts=list(p=pList[[cnt]],pcut=pcut,
            name=cnt),output="json")
	})
	...
	cat("<div class=\"hc-rect-sm\" id=\"chromosome_biodist_container\"></div>",
		sep="")
	cat("<script>$('#chromosome_biodist_container').highcharts(",
		jsonList[["chromosome"]],");</script>",sep="")
	...
	cat("<div class=\"hc-rect-sm\" id=\"biotype_biodist_container\"></div>",
		sep="")
	cat("<script>$('#biotype_biodist_container').highcharts(",
		jsonList[["biotype"]],");</script>",sep="")
}
```
...
for (cnt in contrast) {
	...
	tableId <- paste0("#DataTables_Table_",tableCounter)
	drcl <- paste0("function() {",
...
//console.log($('#'+tids[0]).children("table").attr("id"));

//$('#detable_e14__5_vs_adult_8_weeks_').find("table")[1].id

//console.log(tids[0]);
//console.log($('#'+tids[0]+"_").find("table")[1]);
...
$('a[href="#results"]').on('click',function() {
	console.log("Malakas");
	...
	//console.log(tids[0]+"_");
	//var counter = 0;
	//while ($('#'+tids[0]+"_").find("table")[1] === undefined) {
	//	counter++
	//	console.log(counter);
	//}
	//var id = $('#'+tids[0]+"_").find("table")[1].id;

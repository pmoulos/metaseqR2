...
biotypes <- as.character(geneData$biotype)
...
statPlots <- c("deheatmap","volcano","biodist","venn")
...
if ("biodetection" %in% qcPlots) {
	...
	if (reportDb == "sqlite") {
		# Initialize first plot with triggering
		#cat("<script>$('#biodetection_selector').trigger('change');</script>")
	}
	else if (reportDb == "dexie") {
...
if ("boxplot" %in% qcPlots) {
	...
	cat("<div class=\"hc-rect-sm\" id=\"boxplot_norm_container\"></div>",
		sep="")
	cat("<script>$('#boxplot_unorm_container').highcharts(",jsonUnorm,
		");</script>",sep="")
	cat("<br/><br/>")
	cat("<script>$('#boxplot_norm_container').highcharts(",jsonNorm,
		");</script>",sep="")
}
```
...
if ("volcano" %in% qcPlots) {
	...
	items <- paste("<option value=",paste("volcano",nn,sep="_"),">",
...
if ("mastat" %in% qcPlots) {
	...
	# Create a selector for each contrast
	items <- paste("<option value=",paste("mastat",nn,sep="_"),">",
		gsub("_vs_"," vs ",nn),"</option>",sep="")
	cat("<div>")
	cat("<div class=\"hc-selector-label\">Select a contrast to display plot ",
		"for </div>",sep="")
...
if ("deheatmap" %in% qcPlots) {
	...
	for (cnt in names(contrastList)) {
		...
		hList[[cnt]] <- heatmaply(nat2log(mat),trace="none",colors=bluered(64),
			labRow=rep(NA,nrow(mat)),main=paste("DEG heatmap",con)) %>%

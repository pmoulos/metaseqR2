...
getPlotWithFunFromDb = function(query,renderId) {
	db.plots.get({
		...
		if (query.type === "countsbio") {
			chartData.chart.yAxis.labels.formatter = 
				new Function('return ' +
					chartData.chart.yAxis.labels.formatter)()
				
			chart.plotOptions.tooltip.pointFormatter = 
				new Function('return ' +
					chart.plotOptions.tooltip.pointFormatter)()
			
			chart.plotOptions.events.legendItemClick = 
				new Function('return ' + 
					chart.plotOptions.events.legendItemClick)()
		}
...
```{r analysis_figures_saturation,eval=FALSE}
if ("saturation" %in% qcPlots) {
	...
	for (s in unlist(sampleList)) {
		counter <- counter + 1
		cat("<div class=\"hc-rect-sm\" id=\"saturation_sample_container_",
		    counter,"\"></div>",sep="")
	...
	for (b in names(jsonList[["biotype"]])) {
		counter <- counter + 1
		cat("<div class=\"hc-rect-sm\" id=\"saturation_biotype_container_",
		    counter,"\"></div>",sep="")
...
```{r analysis_figures_readsnoise,eval=FALSE}
...
```{r analysis_figures_correl,eval=FALSE}
if ("correl" %in% qcPlots) {
	...
	cat("<div class=\"figure-hint\">",reportMessages$explain$correl,
	    "<br/><br/></div>",sep="")
	#cat("<div class=\"hc-rect-sm\" id=\"correl_container\"></div>",
	#	sep="")
...
```{r analysis_figures_pairwise,eval=FALSE}
...
```{r analysis_figures_filtered,eval=FALSE}
...
```{r analysis_figures_boxplot,eval=FALSE}
...
```{r analysis_figures_gcbias,eval=FALSE}
...
```{r analysis_figures_lengthbias,eval=FALSE}
...
$("#countsbio_sample_selector").on('change',function() {
	var name = this.value;
	getPlotFromDb({name: name,type: "countsbio",subtype: "sample"},
		"countsbio_sample_container");
});

$("#countsbio_biotype_selector").on('change',function() {
	var name = this.value;
	getPlotFromDb({name: name,type: "countsbio",subtype: "biotype"},

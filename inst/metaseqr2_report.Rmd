...
<!---
<button id="generate_mds">Generate MDS!</button>
<script>
$("#generate_mds").click(function() {	
	//getPlotFromDb('SELECT json FROM plot WHERE type="mds"',"mds_container")
...
## Normalization assessment figures {.tabset .tabset-pills}

The following figures allow for the assessment of the normalization procedures 
performed by the ```metaseqr2``` pipeline. Each figure category is accompanied
by an explanatory text. All figures are interactive wih additional controls on
the top right of the figure.
...
The following figures allow for the assessment of the statistical testing 
procedures performed by the ```metaseqr2``` pipeline. Each figure category is 
accompanied by an explanatory text. All figures are interactive wih additional 
controls on the top right of the figure.
...
if ("deheatmap" %in% qcPlots) {
	...
	# Create a selector for each contrast
	items <- paste("<option value=",paste("deheatmap",names(contrastList),
		sep="_"),">",gsub("_vs_"," vs ",nn),"</option>",sep="")
	...
	for (cnt in names(contrastList)) {
		...
		hList[[cnt]] <- heatmaply(nat2log(mat),trace="none",colors=bluered(64),
			labRow=rep(NA,nrow(mat)),main=paste("DEG heatmap",con,runif(1))) %>%
			layout(width=720,height=840)
	}
	setNames(hList,NULL)
	# FIXME: Attach tag to heatmap before publishing
	htmltools::tagList(
		tags$div(id="test1",class="deheatmap-container",hList[[1]])
	)
	#htmltools::tagList(tags$div(hList))
...
for (cnt in contrast) {
	...
	cat("<div class=\"figure-hint\">The following table presents ",topText,
		" for the contrast <strong>",cntn,"</strong>. The fields of the table ",
		"correspond to the requested features to be exported. The table can ",
		"be searched using the search field on the top right.</div>",
		sep="")
	
	# Fold change should always exist in this table defined by us...
	...
	brks <- quantile(reportTables[[cnt]][,fi],probs=seq(.05,.95,.05),
		na.rm=TRUE)
	preClrs <- col2rgb(redgreen(length(brks)+1))
	clrs <- paste("rgb(",apply(preClrs,2,paste0,collapse=","),")",sep="")
	pRangeForColor <- 1-as.numeric(reportTables[[cnt]][,pi])
	#pZero <- which(pRangeForColor == 0)
	#if (length(pZero) > 0)
	#	pRangeForColor[pZero] <- min(pRangeForColor[-pZero])
	fRangeForColor <- 1-as.numeric(reportTables[[cnt]][,di])
	#fZero <- which(fRangeForColor == 0)
	#if (length(fZero) > 0)
	#	fRangeForColor[fZero] <- min(fRangeForColor[-fZero])
	#pRangeForColor <- 1-pRangeForColor
	#fRangeForColor <- 1-fRangeForColor
	...
	reportTables[[cnt]] <- cbind(pRangeForColor,fRangeForColor,reportTables[[cnt]])
	...
	#) %>% formatStyle(
		#names(reportTables[[cnt]])[di+2],
		#background=styleColorBar(fRangeForColor,"lightsalmon"),
		#fontWeight="bold",
		#backgroundSize="98% 88%",
		#backgroundRepeat="no-repeat",
		#backgroundPosition="center"
	...
	N <- names(sampleList)
	#for (i in 1:length(N)) {
	#	theTable <- formatStyle(
	#		theTable,N[i],
	#		background=styleColorBar(range(reportTables[[cnt]][,N[i]]),
	#			sColors[i]),
	#		#fontWeight="bold",
	#		backgroundSize="98% 88%",
	#		backgroundRepeat="no-repeat",
...
Related references will be displayed here.

...
Welcome to metaseqR2 report! If you are familiar with metaseqR report, then you
will find out that there are not so many differences with respect to the 
...
if ("foldvenn" %in% qcPlots) {
	...
	# Create a selector for each contrast
	nn <- names(contrastList)
	items <- paste("<option value=",paste("foldvenn",nn,sep="_"),">",
		gsub("_vs_"," vs ",nn),"</option>",sep="")
	cat("<div>")
	cat("<div class=\"hc-selector-label\">Select a contrast to display Venn ",
		"diagram for </div>",sep="")
	cat("<select id=\"foldvenn_selector\">",items,"</select>",sep="")
	cat("</div>")
...
$("#foldvenn_selector").on('change',function() {
	var name = this.value;
	getVennFromDb({name: name,type: "foldvenn",subtype: "fold"},
...
if ($("#foldvenn_selector").length > 0) {
	$("#foldvenn_selector").trigger('change');
}
```

...
### `r if ("lengthbias" %in% qcPlots) 'LengthBias'`
...
### `r if ("meandiff" %in% qcPlots) 'MeanDiff'`

```{r analysis_figures_meandiff,eval=FALSE}
if ("meandiff" %in% qcPlots) {
	...
	cat("<div class=\"hc-rect-sm\" id=\"meandiff_norm_container\"></div>",
		sep="")
	#cat("</div></div>")
	cat("<script>$('#meandiff_unorm_container').highcharts(",jsonUnorm[[1]][[1]],
		");</script>",sep="")
	cat("<script>$('#meandiff_norm_container').highcharts(",jsonNorm[[1]][[1]],
		");</script>",sep="")
		
	# Add extra data, serialize and import the plots to Dexie per sample
	preImportUnorm <- vector("list",np)
	index <- 0
	for (i in 1:length(jsonUnnorm)) {
		for (j in 1:length(jsonUnnorm[[i]])) {
			index <- index + 1
			preImportUnorm[[index]] <- list(
				name=names(jsonUnnorm[[i]])[j],
				type="meandiff",
				subtype="unorm",
				json=jsonUnorm[[i]][[j]]
			)
...
### `r if ("meanvar" %in% qcPlots) 'MeanVar'`

```{r analysis_figures_meanvar,eval=FALSE}
if ("meanvar" %in% qcPlots) {
	...
	cat("<div class=\"figure-hint\">",reportMessages$explain$meanvar,
		"<br/><br/></div>",sep="")
	cat("<div class=\"hc-rect-xs\" id=\"meanvar_unorm_container\"></div>",
		sep="")
	cat("<div class=\"hc-rect-xs\" id=\"meanvar_norm_container\"></div>",
		sep="")

...
<!--<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/sql.js/0.5.0/js/sql.js"></script>-->
...
$("#generate_mds").click(function() {	
	...
	getPlotFromDb({name: "MDS",type: "mds",subtype: null},"mds_container");
})
</script>
...
if ("mds" %in% qcPlots) {
	...
	cat("db.plots.add({name: 'MDS',type: 'mds', subtype: null, json: ",json,"});",sep="")
	cat("</script>")
}
```
...
if ("biodetection" %in% qcPlots) {
	...
	for (s in unlist(sampleList)) {
		counter <- counter + 1
		cat("<div class=\"hc-rect-sm\" id=\"biodetection_container_",counter,
		    "\"></div>",sep="")
...
if ("countsbio" %in% qcPlots) {
	...
	for (s in unlist(sampleList)) {
		counter <- counter + 1
		cat("<div class=\"hc-rect-sm\" id=\"countsbio_sample_container_",
		    counter,"\"></div>",sep="")
	...
	for (b in names(jsonList[["biotype"]])) {
		counter <- counter + 1
		cat("<div class=\"hc-rect-sm\" id=\"countsbio_biotype_container_",
		    counter,"\"></div>",sep="")

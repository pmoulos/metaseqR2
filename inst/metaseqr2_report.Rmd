...
if ("correl" %in% qcPlots) {
	...
	setNames(h,NULL)
	htmltools::tagList(tags$div(class="deheatmap-container",h[[1]]))
}
```
...
$("#biodetection_selector").on('change',function() {
	var name = this.value;
	if (DEXIE) {
		getPlotFromDexieDb({name: name,type: "biodetection",subtype: "generic"},
...
$("#countsbio_sample_selector").on('change',function() {
	var name = this.value;
	if (DEXIE) {
		getPlotWithFunFromDexieDb({name: name,type: "countsbio",
			subtype: "sample"},"countsbio_sample_container");
	}
	else {
...
$("#countsbio_biotype_selector").on('change',function() {
	...
	if (DEXIE) {
		getPlotWithFunFromDexieDb({name: name,type: "countsbio",
			subtype: "biotype"},"countsbio_biotype_container");
	}
	else {
...
$("#_mds_trigger").on('click',function() {
	if (DEXIE) {
		getPlotFromDexieDb({name: "MDS",type: "mds",subtype: "generic"},
			"mds_container");
	}
	else {
...
$("#_readnoise_trigger").on('click',function() {
	if (DEXIE) {
		getPlotFromDexieDb({name: "ReadNoise",type: "readnoise",subtype: "generic"},
